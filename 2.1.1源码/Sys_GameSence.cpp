...
{
    setWindowTitle("超级可莉");//设置标题
    setFixedSize(800, 545);//设置窗口大小
    Game_Init();
    Pause_Init();
    gsmusicList = new QMediaPlaylist;  //添加音乐列表
    gsmusicList->addMedia(QUrl("qrc:/music/bgm.mp3"));//添加音乐列表
    gsmusicList->setPlaybackMode(QMediaPlaylist::CurrentItemInLoop);
    gsmusicList->setCurrentIndex(-1);
    gsmusicplay=new QMediaPlayer;
    gsmusicplay->setPlaylist(gsmusicList);
    gsmusicplay->setVolume(8);
    gsmusicplay->play();
...
void GameScene::Pause_Init()
{
    ...
    MyPushButton *btn_exit = new MyPushButton(":/photo/end.png");//添加离开按钮
...
void GameScene::paintEvent(QPaintEvent *)
{
    ...
    if (!game_start) {
        ...
        painter.drawText(40, 30, "times:");
        painter.drawText(150, 32, QString::number(time, 'f', 1));
...
void GameScene::Jump_Collision() {
    ...
    it++)
    {
        if (*it->begin() - mary->x - 300 >= -30 && *it->begin() - mary->x - 300 <= 30 &&
            *(it->begin() + 1) - mary->y + 40 >= -10 && *(it->begin() + 1) - mary->y + 40 <= 20) {
            if (*(it->begin() + 2) == 1) {
                QMediaPlaylist *comusicList = new QMediaPlaylist;  //添加音乐列表
                comusicList->addMedia(QUrl("qrc:/music/coin.mp3"));//添加音乐列表
                comusicList->setCurrentIndex(1);
                QMediaPlayer *comusicplay=new QMediaPlayer;
                comusicplay->setPlaylist(comusicList);
                comusicplay->setVolume(7);
                comusicplay->play();
                unknown->coin++;
                score += 10;
...
void GameScene::Die_Init() {
    ...
    if (mary->y > 500) {
        mary->y = 455;
        QMediaPlaylist *diemusicList = new QMediaPlaylist;  //添加音乐列表
        diemusicList->addMedia(QUrl("qrc:/music/die.mp3"));//添加音乐列表
        diemusicList->setCurrentIndex(1);
        QMediaPlayer *diemusicplay=new QMediaPlayer;
        diemusicplay->setPlaylist(diemusicList);
        diemusicplay->setVolume(25);
        diemusicplay->play();
        mary->life--;
        killTimer(timer3);
        killTimer(timer1);
