...
void MushRoom::Move_state(){
    ...
    if (mushroom_y >= 460 && mushroom_state != 0)
    {
        ...
        {
            mushroom_state = 0;
            QMediaPlaylist *p1musicList = new QMediaPlaylist;  //添加音乐列表
            if(mary->colour==1)
            {
                p1musicList->addMedia(QUrl("qrc:/music/palyer1.mp3"));//添加音乐列表
            }
            else
            {
                p1musicList->addMedia(QUrl("qrc:/music/player2.mp3"));//添加音乐列表
            }
            p1musicList->setCurrentIndex(1);
            QMediaPlayer *p1musicplay=new QMediaPlayer;
            p1musicplay->setPlaylist(p1musicList);
            p1musicplay->setVolume(30);
            p1musicplay->play();
